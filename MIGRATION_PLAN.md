...
| 5 — KML import/export | ⬜ Not started | Admin KML upload sub-panel is a lazy-load candidate. **Also now owns:** the save modal (`#saveModal`, `openSaveModal`, app.js ~2033–2306) and export builders (`buildGPX`/`buildKML`/`buildGeoJSON`/`downloadFile`) — moved here from Slice 4's literal line range once tracing call sites showed `openSaveModal()`'s only caller is `processImportPipeline` (~line 1895), so it can't function without this slice's import pipeline existing first. See Slice 4's row for the full rationale. Re-target `saveToFbBtn`'s Firebase write to a Supabase `segments` insert — do not port `window.db`/`firebase.firestore.FieldValue.serverTimestamp()` as-is. |
